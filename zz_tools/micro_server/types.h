...
#define ZZ_FWD(...) ::std::forward<decltype(__VA_ARGS__)>(__VA_ARGS__)
...
using case_sensitive_map = std::map<std::string, nlohmann::json>;
...
std::string to_string(const zz_tools::request_method method);
...
struct response
{
    ...
    response(::zz_tools::status_code status_code) : status_code{status_code}, file{""}
    { }

    response(::zz_tools::status_code status_code, const ::zz_tools::request_headers& headers) : status_code{status_code}, headers{headers}, file{""}
    { }

    response(::zz_tools::status_code status_code, std::string content) : status_code{status_code}, content{content}, file{""}
    { }

    response(::zz_tools::status_code status_code, const ::zz_tools::request_headers& headers, std::string content) : status_code{status_code}, headers{headers}, content{content}, file{""}
    { }

    response(::zz_tools::status_code status_code, std::string content_type, std::string content) :
            status_code{status_code}, content_type{content_type}, content{content}, file{""}
    { }

    response(::zz_tools::status_code status_code, const ::zz_tools::request_headers& headers, std::string content_type, std::string content) :
            status_code{status_code}, headers{headers}, content_type{content_type}, content{content}, file{""}
    { }

    // default success responses
    response(const ::zz_tools::request_headers& headers, std::string content) : status_code{0}, headers{headers}, content{content}, file{""}
    { }

    response(const ::zz_tools::request_headers& headers) : status_code{0}, headers{headers}, file{""}
    { }

    response(std::string content) : status_code{0}, content{content}, file{""}
    { }

    response(const ::zz_tools::request_headers& headers, std::string content_type, std::string content) :
    ...
    response(::zz_tools::status_code status_code, URI redirect) :
...
struct validator
{
    ...
    validator(std::string &&key, bool required, validation_function validation_func=nullptr) : key{std::move(key)}, required{required}, validation_func{validation_func} {};
    validator(const std::string &key, bool required, validation_function validation_func=nullptr) : key{key}, required{required}, validation_func{validation_func} {};
