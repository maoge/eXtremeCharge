...
namespace ZZTools
{
	IPCID_T IPCMSQ::Create(const IPCKEY_T key,
	...
	IPCID_T IPCMSQ::Create()
	{
		return Create(IPC_PRIVATE, CREATE_NEW_ONE, IPC_DEFAULT_PERM);
	}

	IPCID_T IPCMSQ::GetID(const IPCKEY_T key)
	{
	...
	Buffer_ptr IPCMSQ::Receive(const IPCID_T id, long type /*= 0*/, bool blocked /*= true*/)
	{
		int retcode = 0;
		int flg = 0;
...
MSGRCV:
	...
	IPCMSQ_RET_TYPE_E IPCMSQ::SendMsg(IPCID_T id, const char*buf, int length, bool blocked /*= true*/)
	{
		int nFlag = 0;
	...
	IPCMSQ_RET_TYPE_E IPCMSQ::ReceiveMsg(IPCID_T id, Buffer& msg, bool blocked /*= true*/)
	{
		int nFlag = 0;
