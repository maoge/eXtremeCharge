...
namespace ZZTools
{
	...
	std::string Utility::DateTimeNow()
	{
		...
		if(NULL != nowTime)
		{
			sprintf(nowBuf, "%d/%02d/%02d %02d:%02d:%02d.%03ld",
			        nowTime->tm_year + 1900,
			        nowTime->tm_mon + 1,
			        nowTime->tm_mday,
			        nowTime->tm_hour,
	...
	long Utility::GetMillisecondsFromLocal()
	{
		struct timeval tv = {0};
		gettimeofday(&tv, NULL);
	...
	TIME_PRECISION_T Utility::GetTimePrecision()
	{
		...
		tp.nSeconds = (UINT32)tv.tv_sec;
		tp.nUSeconds = (UINT32)tv.tv_usec;

		return tp;
	}

	TIME_PRECISION_T Utility::GetBootTime()
	{
		return _nBootTime;
	}
	...
	UINT16 Utility::IntegerToLocalByte16(UINT16 value, bool bigendian)
	{
		bool bSwap = false;
	...
	UINT32 Utility::IntegerToLocalByte32(UINT32 value, bool bigendian)
	{
		bool bSwap = false;
#ifdef LWPR_BIG_ENDIAN
		bSwap = !bigendian;
	...
	UINT64 Utility::IntegerToLocalByte64(UINT64 value, bool bigendian)
	{
		bool bSwap = false;
#ifdef LWPR_BIG_ENDIAN
		bSwap = !bigendian;
	...
	UINT64 SWAP_BYTE_ORDER64(UINT64 value)
	{
		...
		return ((((UINT64)(value) & (tmp << (8 * 7))) >> 56) |
		        (((UINT64)(value) & (tmp << (8 * 6))) >> 40) |
		        (((UINT64)(value) & (tmp << (8 * 5))) >> 24) |
		        (((UINT64)(value) & (tmp << (8 * 4))) >> 8)  |
		        (((UINT64)(value) & (tmp << (8 * 3))) << 8)  |
		        (((UINT64)(value) & (tmp << (8 * 2))) << 24) |
		        (((UINT64)(value) & (tmp << (8 * 1))) << 40) |
		        (((UINT64)(value) & (tmp << (8 * 0))) << 56));
	}
};
