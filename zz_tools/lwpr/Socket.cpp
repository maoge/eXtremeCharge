...
namespace ZZTools
{
	...
	//----------------------------------------------------------------------------------
	void socket_addr_t::Marshal(OutputStream& out)
	{
		out.WriteUINT32(ip);
		out.WriteUINT16(port);
	}

	void socket_addr_t::Unmarshal(InputStream& in)
	{
		ip = in.ReadUINT32();
		port = in.ReadUINT16();
	}
	...
	// class Socket
	//----------------------------------------------------------------------------------
	SOCKET_FD_T Socket::CreateServerTCPSocket(const char* host, int port)
	{
		assert(host != NULL);
		assert(port >= 0);
	...
	SOCKET_FD_T Socket::CreateServerUDPSocket(UINT16& port)
	{
	...
	SOCKET_FD_T Socket::CreateClientUDPSocket()
	{
		SOCKET_FD_T fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
		return fd;
	}

	SOCKET_FD_T Socket::CreateServerTCPSocket(const char* addr)
	{
		assert(NULL != addr);
		std::string host;
		int min_port = 0;
		...
		if(SplitAddr(addr, host, min_port, max_port))
		{
			...
			{
				SOCKET_FD_T nFd = CreateServerTCPSocket(host.c_str(), i);
				if(nFd != -1)
				{
					return nFd;
				}
	...
	SOCKET_FD_T Socket::AcceptSocket(SOCKET_FD_T fd)
	{
		assert(fd > 0);
	...
	SOCKET_FD_T Socket::ConnectRemoteHost(const char* host, int port)
	{
		assert(host != NULL);
		assert(port > 0);
	...
	SOCKET_FD_T Socket::ConnectRemoteHost(const char* addr)
	{
		assert(NULL != addr);
		std::string host;
		int min_port = 0;
	...
	SOCKET_RET_TYPE_E Socket::ReadSocket(SOCKET_FD_T fd, Buffer& buf, int length, int timeout)
	{
		buf.Size(length);
	...
	SOCKET_RET_TYPE_E Socket::ReadSocket(SOCKET_FD_T fd, Buffer& buf, int length)
	{
		buf.Size(length);
	...
	SOCKET_RET_TYPE_E Socket::ReadSocket(SOCKET_FD_T fd, char* buf, int length, int timeout)
	{
	...
	SOCKET_RET_TYPE_E Socket::ReadSocket(SOCKET_FD_T fd, char* buf, int length)
	{
		return ReadSocket(fd, buf, length, INT_MAX);
	}

	SOCKET_RET_TYPE_E Socket::WriteSocket(SOCKET_FD_T fd, const char* buf, int length, int timeout)
	{
		int nDoneLen = 0, nRetNum = 0;
		SOCKET_RET_TYPE_E nRetResult = SOCKET_RET_FAILED;
	...
	SOCKET_RET_TYPE_E Socket::WriteSocket(SOCKET_FD_T fd, const char* buf, int length)
	{
		return WriteSocket(fd, buf, length, INT_MAX);
	}

	SOCKET_RET_TYPE_E Socket::IsSocketReadable(SOCKET_FD_T fd, int timeout)
	{
		assert(fd >= 0);
		assert(timeout >= 0);
	...
	SOCKET_RET_TYPE_E Socket::IsSocketReadable(SOCKET_FD_T fd)
	{
		return IsSocketReadable(fd, INT_MAX);
	}

	SOCKET_RET_TYPE_E Socket::IsSocketWritable(SOCKET_FD_T fd, int timeout)
	{
		assert(fd >= 0);
		assert(timeout >= 0);
	...
	SOCKET_RET_TYPE_E Socket::IsSocketWritable(SOCKET_FD_T fd)
	{
	...
	void Socket::AddrTransform(const SOCKET_ADDR_T& addr_src, std::string& addr_dest)
	{
		...
		UINT32 port = htons(addr_src.port);
	...
	SOCKET_RET_TYPE_E Socket::WriteUDPSocket(SOCKET_FD_T fd, const char* addr, const char* buf, int length)
	{
		assert(addr != NULL);
		assert(buf != NULL);
	...
	SOCKET_RET_TYPE_E Socket::WriteUDPSocket(SOCKET_FD_T fd, int port, const char* buf, int length)
	{
		char addrbuf[64] = {0};
	...
	void Socket::GetLocalAddr(int fd, ADDR_T& local_addr)
	{
		if(fd < 0)
			logger->info(LTRACE, "mqnode fd erro!");
		sockaddr_in addr;
				
		memset(&addr,0,sizeof(addr));
		int len = sizeof(addr);
		int ret = getsockname(fd, (sockaddr*)&addr, (socklen_t*)&len);
		if (ret != 0) {
			logger->error(LTRACE, "getsockname erro!");
		} else {
			memset(&local_addr, 0, sizeof(local_addr));
			sprintf(local_addr.ip, "%s", inet_ntoa(addr.sin_addr));
			local_addr.port = ntohs(addr.sin_port);
			logger->info(LTRACE, "local_addr  %s  : %d",local_addr.ip, local_addr.port);
		}
	}

	void Socket::GetPeerAddr(int fd, ADDR_T& server_addr)
	{
		if(fd < 0)
			logger->info(LTRACE, "mqnode fd erro!");
		sockaddr_in addr;
		...
		memset(&addr,0,sizeof(addr));
		int len = sizeof(addr);
		int ret = getpeername(fd, (sockaddr*)&addr, (socklen_t*)&len);
		if (ret != 0) {
			logger->error(LTRACE, "getpeername erro!");
		} else {
			memset(&server_addr, 0, sizeof(server_addr));
			sprintf(server_addr.ip, "%s", inet_ntoa(addr.sin_addr));
			server_addr.port = ntohs(addr.sin_port);
			logger->info(LTRACE, "server_addr  %s  : %d",server_addr.ip, server_addr.port);
		}
	}
	...
	bool Socket::IsLocalPortUsed(int port)
	{
		SOCKET_FD_T fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		bool bRetResult = (fd != -1);
		if(!bRetResult) {
	...
	bool Socket::IsLocalPortUsed(const char* host, int port)
	{
		SOCKET_FD_T fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		bool bRetResult = (fd != -1);
		if(!bRetResult) {
