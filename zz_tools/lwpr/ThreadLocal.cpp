...
namespace ZZTools
{
		...
		: m_Key(0)
	{
		...
		if(retcode != 0)
			throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_key_create error");
	}
	...
	void ThreadLocal::SetValue(TLSValue* value)
	{
		...
		if(retcode != 0)
			throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_setspecific error");
	}
