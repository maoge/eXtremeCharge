...
namespace ZZTools
{
		...
		: m_nWaitThreads(0), m_nResource(0)
	{
		...
		if(retcode != 0)
			throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_cond_init error");
	}
	...
	void Resource::Wait()
	{
		...
		while(m_nResource == 0)
		{
			...
			{
				m_nWaitThreads--;
				throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_cond_wait error");
			}
		}
	...
	void Resource::Wait(int seconds)
	{
		...
		while(m_nResource == 0)
		{
			...
			{
				m_nWaitThreads--;
				throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_cond_timedwait error");
			}
		}
	...
	void Resource::Notify()
	{
		...
		if(retcode != 0) throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_cond_signal error");
	...
	void Resource::Notifyall()
	{
		...
		if(m_nWaitThreads > 0)
		{
			...
			if(retcode != 0) throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_cond_broadcast error");
