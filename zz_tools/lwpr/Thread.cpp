...
namespace ZZTools
{
		  ...
		  m_bContinue(false)
	{
		...
		if(retcode != 0)
			throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_attr_init error");
	}
	...
	void Thread::Start()
	{
		...
		if(!m_bStarted)
		{
			...
			if(retcode != 0)
				throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_create error");
	...
	THREAD_ID_T Thread::GetCurrentThreadId()
	{
		return pthread_self();
	}
	...
	void Thread::EnableJoinable()
	{
		if(!m_bStarted)
		{
			...
			if(retcode != 0)
			{
				...
				pthread_attr_destroy(&m_threadAttr);
				throw LWPR_THREAD_API_ERR(EXCEPTION_THREAD_TRACE, retcode, "pthread_attr_setdetachstate error");
			}
		}
	}
